...
class Search {
      ...
      this.resultsDiv.html(`
		 <div class="rpw">
		 	<div class="one-third">
		 		<h2 class="search-overlay__section-title">General Information</h2>
		 		 ${results.generalInfo.length ? '<ul class="link-list min-list">' : '<p>No general information matches that match.</p>'}
		  	 	 ${results.generalInfo.map(item => `<li><a href="${item.permalink}">${item.title}</a> ${item.postType == 'post' ? `by ${item.authorName}` : ''}</li>`).join('')}
		  		${results.generalInfo.length ? '</ul>' : ''}
		 	</div>
		 	<div class="one-third">
		 		<h2 class="search-overlay__section-title">Programs</h2>
		 		 ${results.programs.length ? '<ul class="link-list min-list">' : `<p>No programs information match.<a href="${universityData.root_url}/programs">View all programs.</a></p>`}
		  	 	 ${results.programs.map(item => `<li><a href="${item.permalink}">${item.title}</a></li>`).join('')}
		  		${results.programs.length ? '</ul>' : ''}
		 		
		 		<h2 class="search-overlay__section-title">Professors</h2>
		 		
		 		${results.professors.length ? '<ul class="professor-cards">' : `<p>No professors information match.</p>`}
		 ...
		 `).join('')}
		 	...
		 	<div class="one-third">
		 		...
		 		<p>It could not work due to the Google's policy on googleMap API:(.</p>
		 		<h2 class="search-overlay__section-title"> Events</h2>
		 		<!--template-parts/content-eventDOTphp-->
		 		${results.events.length ? '' : `<p>No events information match that match.<a href="${universityData.root_url}/events">View all events.</a></p>`}
  ...
  <div class="event-summary">
		 ...
		 `);
      this.isSpinnerVisible = false; //fix the spinning icon after you change typed value.
    });
  }

  keyPressDispatcher(e) {
    if (e.keyCode == 83 && !this.isOverlayOpen && !jquery__WEBPACK_IMPORTED_MODULE_0___default()("input, textarea").is(':focus')) {
      //If any field is currently focus, this will be valuated as false
      this.openOverlay();
    }
  ...
  openOverlay() {
    ...
    /*To autofocus the search field once we open the search layout.*/
    , 301);
